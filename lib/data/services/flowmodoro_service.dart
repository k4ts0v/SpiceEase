...
class FlowmodoroService {
  final FlowmodoroRepository _repository; // Dependency for accessing the repository.

  const FlowmodoroService(this._repository, AuthService auth, DatabaseService db);

  /// Retrieves all flowmodoro by delegating to the repository.
  Future<List<FlowmodoroModel>> getAllFlowmodoro() => _repository.getAllFlowmodoro();

  /// Retrieves a specific flowmodoro by ID through the repository.
  Future<FlowmodoroModel?> getFlowmodoroById(String id) => _repository.getFlowmodoroById(id);

  /// Creates a new flowmodoro by delegating to the repository.
  Future<FlowmodoroModel> createFlowmodoro(FlowmodoroModel flowmodoro) => _repository.createFlowmodoro(flowmodoro);

  /// Updates an existing flowmodoro by delegating to the repository.
  Future<FlowmodoroModel> updateFlowmodoro(String id, FlowmodoroModel flowmodoro) =>
  ...
  Future<List<FlowmodoroModel>> getFlowmodorosForDate(DateTime date) => _repository.getFlowmodorosForDate(date);
}
